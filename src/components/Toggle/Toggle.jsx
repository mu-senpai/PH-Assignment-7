        ...
        <div className="max-w-[95%] sm:max-w-[90%] lg:max-w-[81.25%] min-[1920px]:max-w-[97.5rem] mx-auto">
            ...
            <div className="w-full flex flex-col sm:flex-row items-center gap-4 sm:gap-0 justify-center sm:justify-between mb-8 sticky z-10 top-5 sm:top-[5.7rem] lg:top-[6.9rem]">
                ...
                <div className="flex items-center border border-[#1313131A] rounded-xl">
                    <div onClick={() => {handleIsActive('available'); scrollToComponent();}} className={`w-[45%] px-6 py-3 text-xs sm:text-sm lg:text-base border-r border-[#1313131A] rounded-l-xl ${isActive ? 'bg-[#E7FE29] text-black font-bold' : 'bg-white text-[#13131399] font-normal'}`}>Available</div>
                    <div onClick={() => {handleIsActive(''); scrollToComponent()}} className={`w-[55%] px-6 py-3 text-xs sm:text-sm lg:text-base rounded-r-xl ${!isActive ? 'bg-[#E7FE29] text-black font-bold' : 'bg-white text-[#13131399] font-normal'}`}>Selected ({selectedPlayers.length})</div>
                </div>    
            </div>
